...
Then it slices the file content (BLOB) into chunks of a selected size and sends chunks in iterations (as a part of FormData) to the service vi HTTP POST request.
The service reads a chunk, appends it to the destination file and returns the number of bytes written.
After each iterations, the clien displays how many bytes have been uploaded.
In case of error, client app displays it.
